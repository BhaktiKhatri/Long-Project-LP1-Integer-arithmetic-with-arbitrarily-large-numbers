...
public class Num implements Comparable<Num> {
	 ...
	 */
	static Num product_(Num a, Num b) {
		int shiftSize = 0;
	...
	static Num divide(Num a, Num b) {
		...
		Num zero=new Num(0,a.base),one=new Num(1L, a.base),two=new Num(2L, a.base);
		Num quotient = zero;
		boolean aZero = false;
		boolean bZero = false;
		...
		try {
			...
			if(a.compareTo(b) == -1 && !a.sign && !aZero) {
			...
			else if(b.compareTo(one) == 0 && a.compareTo(b)!= 0) {
				quotient = a;
			}
			else {
				...
				while((high.compareTo(low) == 1) || (high.compareTo(low) == 0)) {
					...
					while((high.compareTo(product(temp,two)) == 1) || high.compareTo(product(temp,two)) == 0)	{
						temp = product(temp,two);
						multiple = product(multiple,two);
					}
				...
				if((!b.sign && a.sign) || (b.sign && !a.sign)) {
					quotient = subtract(zero, quotient);
				} 
			}
	...
	static Num squareRoot(Num a) {
		...
		if(size == 1 && (a.compareTo(zero) == 0 || a.compareTo(one) == 0))
			return a;
		Num low = zero;
		Num high = divide(a, two);
		Num mid=zero,sMid;
		while(!(((product(mid,mid).compareTo(a)==-1)||(product(mid,mid).compareTo(a)==0))&&(product(add(mid,one),add(mid,one)).compareTo(a)==1))){
			//Num mid = divide(add(low, high), new Num(2L, a.base));
			mid=divide(subtract(high,low),two);
			mid=add(low,mid);
			sMid = product(mid, mid);
